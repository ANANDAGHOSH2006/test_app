...
def seed_from_json(connection):
    ...
    try:
        if OPERATIONS_FILE.exists():
            loaded = json.loads(OPERATIONS_FILE.read_text(encoding="utf-8"))
    ...
    for table, columns in tables.items():
        if connection.execute(f"SELECT 1 FROM {table} LIMIT 1").fetchone():
            continue
        records = operations.get(table, DEFAULT_DATA[table])
        if not isinstance(records, list):
            records = DEFAULT_DATA[table]
        records = [record for record in records if isinstance(record, dict) and all(column in record for column in columns)]
        placeholders = ", ".join("?" for _ in columns)
        connection.executemany(f"INSERT INTO {table} ({', '.join(columns)}) VALUES ({placeholders})", [[item[column] for column in columns] for item in records])
...
def analyze_geospatial(state="", district="", risk_layer=""):
    initialize_database()
    clauses, params = [], []
    ...
    if district and district.lower() != "all":
        clauses.append("l.district = ?")
        params.append(district)
    if risk_layer and risk_layer.lower() not in {"all", "all risk levels"}:
        clauses.append("rp.risk = ?")
        params.append(risk_layer.title())
    where = f"WHERE {' AND '.join(clauses)}" if clauses else ""
    query = f"""
        SELECT rp.id, rp.name, rp.risk, rp.latitude AS lat, rp.longitude AS lng,
            l.district, l.state
        FROM risk_points rp
        LEFT JOIN locations l ON l.district = rp.name
        {where}
        ORDER BY rp.id
    """
    with connect() as connection:
        points = rows_as_dicts(connection.execute(query, params))
    return {"points": points, "layersUpdated": len(points), "message": "Geospatial risk analysis completed."}
    ...
    with connect() as connection:
        for observation in observations:
            ...
            warning = {
                ...
                "issued_at": now,
                "expires_at": (datetime.now(timezone.utc) + timedelta(minutes=result["lead_time_minutes"])).isoformat(timespec="seconds").replace("+00:00", "Z"),
            }
...
def get_audit_events(limit=100):
    ...
    try:
        ...
        limit = 100
    with connect() as connection:
        return rows_as_dicts(connection.execute("SELECT * FROM audit_events ORDER BY created DESC, id DESC LIMIT ?", (limit,)))
